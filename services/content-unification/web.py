...
def load_vocab_file(uri: str, graph: str = MU_APPLICATION_GRAPH):
    ...
    file_result = sparql_query(query_string)['results']['bindings'][0]
...
def run_vocab_unification(file_uri: str, target_graph: str):
...
def run_vocab_unification_req(job_uuid: str):
    ...
    run_job(
        ...
        lambda sources: run_vocab_unification(sources[0], MU_APPLICATION_GRAPH),
        sparql_query,
        sparql_update
    )
